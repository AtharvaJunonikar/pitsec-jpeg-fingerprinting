...
def main():
    ...
    #jpeg_files = jpeg_files[:20]  # Limit to first 20 for testing
    ...
    pbar = tqdm(total=len(jpeg_files), desc="Extracting", unit="image")
    ...
    for jpeg_file in jpeg_files:
        ...
        if features is not None:
            ...
            # Extract version and cluster from filename
            version = extract_version_from_filename(jpeg_file.name)
            features['version'] = version
            
            cluster = get_cluster_label(version)
    ...
    log_message(f"Extracted features from {len(results)} images, {failed_count} failed")
    ...
    print("\nSTEP 6: Saving combined feature CSV...")
